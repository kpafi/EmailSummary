...
| `mail_unparsable` | **ERROR** (Felder `mail` = gekürzter Dedupe-Hash, `error` = Exception-Klasse, z. B. `RecursionError`). Schon die IMAP-Bibliothek konnte die Mail nicht parsen (etwa rund 1000 ineinander gesteckte `message/rfc822`-Teile). MailDigest hat sie je UID isoliert, die Kopfzeilen einzeln nachgeholt und sie wie eine unlesbare Mail behandelt: Notiz, `failed`/`ingest_error`, als gelesen markiert — der Zyklus läuft weiter, ein Neustart hilft nicht und ist nicht nötig. **Kein** Verbindungsproblem: Erscheint stattdessen `ingest_failed` mit `ImapConnectionError`, liegt es am Netz oder Server (O-1, ADR-020-Nachtrag, zweite Iteration) |
| `mail_header_fetch_failed` | WARNING (Feld `error`). Der Kopfzeilen-Abruf zu einer unparsbaren Mail wurde abgelehnt oder ist gescheitert; die Notiz kommt dann ohne Absender/Betreff, der Dedupe-Key hängt allein an der UID. Folgt unmittelbar ein Verbindungsfehler, war die Verbindung hin |
